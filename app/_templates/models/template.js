module.exports = function (app) {


    var orm = require("orm"),
        tesla = require('../../tesla_modules/tesla')(app),
        teslaDB = require('../../tesla_modules/tesla.db')(app),
        enforce = require("enforce"),
        colors = require('colors'),
        db  = orm.connect(app.config.db.url);

    orm.settings.set("instance.returnAllErrors", true);

    // DEFINE MODEL SCHEMA
    // Be sure to add some files to the schema below or you will not have success quering or adding to the database
    var Model = db.define("{model}", {
        created   : { type: "date", time: true },
        updated   : { type: "date", time: true }
        // _id : { type: "text" },
        // name      : { type: "text", required: true },
        // isAdmin : { type: "boolean", defaultValue: false },
    }, {
        validations: {
            // EXAMPLE VALIDATIONS
            // password: orm.enforce.security.password('luns5', 'Passowrd does not meet min security requirements.'),
            // email: orm.enforce.patterns.email('Please enter a valid email address.')
            // More Options : https://github.com/dresende/node-enforce
        }
    });



    return {

        // FIND ALL ITEMS
        all : function ( options, cb ) {

            var opt = teslaDB.options ( options );

            db.on("connect", function (err) {

                // IF WE FAIL TO GET A CONNECTION
                if (!err) {

                    Model.find('', opt.limit, [ opt.orderBy, opt.order ], function (err, result) {

                        if ( !err ) {

                            var returnData = {
                                status : 'success',
                                msg : result.length + ' results found.',
                                data : result
                            };

                        } else {

                            var returnData = {
                                status : 'error',
                                msg : 'Error querying database.',
                                data : err
                            };

                        }

                        return cb(returnData);

                    });

                // IF WE CONNECT SUCESFULLY
                } else {

                    console.log(err);

                    if ( err.errmsg ) {
                        var msg = err.name + ': ' + err.errmsg + ' - code: ' + err.code;
                    } else {
                        var msg = 'Error connecting to the database.';
                    }

                    var returnData = {
                        status : 'error',
                        msg : msg,
                        data : err
                    };

                    return cb(returnData);

                }

            });

        }, // END ALL

        // CREATE ITEM(S)
        create : function ( data, cb ) {

            data.created = new Date().toJSON();
            data.updated = new Date().toJSON();

            db.on("connect", function (err) {

                // IF WE FAIL TO GET A CONNECTION
                if (err) {
                    console.log("Error connecting to the database.", err);
                    return;

                // IF WE CONNECT SUCESFULLY
                } else {

                    console.log('connected to: ' + app.config.db.url);

                    Model.create( data, function (err, result) {
                        if (err) {

                            if ( err.msg !== undefined) {
                                var msg = 'Invalid or missing value for "' + err.property + '" field (' + err.value + ') - Message: ' + err.msg;
                            } else {
                                var msg = err;
                            }

                            var returnData = {
                                status : 'error',
                                msg : msg,
                                data : err
                            };

                            return cb(returnData);

                        } else {

                            var returnData = {
                                status : 'success',
                                data : result
                            };

                            return cb(returnData);

                        }
                    });

                }

            });

        }, // END CREATE


        // DELETE (ONLY WORKS WITH SPECIFIC ID FOR NOW)
        delete : function( query, cb ) {

            db.on("connect", function (err) {

                // IF WE CONNECT SUCESFULLY
                if (!err) {

                    Model.find( query ).remove(function (err) {

                        // IF WE SUCCEEDED
                        if (!err) {

                            var returnData = {
                                status : 'success',
                                msg : 'Item removed.'
                            };

                        // IF WE FAILED
                        } else {

                            var returnData = {
                                status : 'error',
                                msg : msg,
                                data : err
                            };

                        }

                        return cb(returnData);

                    });

                // IF WE FAIL TO GET A CONNECTION
                } else {

                    var returnData = {
                        status : 'error',
                        msg : "Error connecting to the database.",
                        data : err
                    };

                    return cb(returnData);

                }

            });

        }, // END DELETE

        // QUERY FOR MATCHING ITEM(S)
        find : function ( query, options, cb ) {

            var self = this;

            var opt = teslaDB.options ( options );

            db.on("connect", function (err) {

                // IF WE CONNECT SUCESFULLY
                if (!err) {

                    // console.log('query: ' + query);

                    Model.find(query, opt, function (err, result) {

                        if ( !err ) {

                            var returnData = {
                                status : 'success',
                                msg : result.length + ' results found.',
                                data : result
                            };

                        } else {

                            var returnData = {
                                status : 'error',
                                msg : 'Error querying database.',
                                data : err
                            };

                        }

                        return cb(returnData);

                    });

                // IF WE FAIL TO GET A CONNECTION
                } else {

                    var returnData = {
                        status : 'error',
                        msg : "Error connecting to the database.",
                        data : err
                    };

                    return cb(returnData);

                }


            });

        }, // END FIND

        // UPDATE (ONLY WORKS WITH SPECIFIC ID FOR NOW)
        update : function( _id, updateData, cb ) {


            db.on("connect", function (err) {

                // IF WE CONNECT SUCESFULLY
                if (!err) {

                    console.log('find id: ' + _id);

                    Model.get(_id, function (err, result) {

                        var returnData = {
                            status : 'success',
                            msg : 'Record successfully updated.',
                            data : result
                        };

                        result.save(updateData, function (err) {
                            return cb(returnData);
                        });
                    });

                // IF WE FAIL TO GET A CONNECTION
                } else {

                    console.log('error');

                    var returnData = {
                        status : 'error',
                        msg : "Error connecting to the database.",
                        data : err
                    };

                    return cb(returnData);

                }

            });

        }, // END update
    }

};